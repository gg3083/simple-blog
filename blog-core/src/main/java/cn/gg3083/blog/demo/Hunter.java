package cn.gg3083.blog.demo;

import cn.gg3083.blog.demo.config.HunterConfig;
import cn.gg3083.blog.demo.enums.ExitWayEnum;
import cn.gg3083.blog.demo.exception.HunterException;
...
public class Hunter extends Spider {
     ...
     */
    public static final ConcurrentHashMap<String, me.zhyd.hunter.Hunter> SPIDER_BUCKET = new ConcurrentHashMap<>();
    ...
    public static me.zhyd.hunter.Hunter getHunter(String hunterId) {
        if (StringUtils.isEmpty(hunterId)) {
            throw new HunterException("HunterId：[" + hunterId + "]为空，请指定HunterId");
        }
        me.zhyd.hunter.Hunter hunter = SPIDER_BUCKET.get(hunterId);
        if (null == hunter) {
